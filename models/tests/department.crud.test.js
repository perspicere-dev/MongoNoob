...
describe('Department', () => {
  ...
  describe('Updating data', () => {
    ...
    beforeEach(async () => {
        const testDepOmne = new Department({ name: 'Department #21' });
        await testDepOmne.save();
      
        const testDepTwo = new Department({ name: 'Department #22' });
        await testDepTwo.save();
    });
    ...
    it('should properly update one document with "updateOne" method', async () => {
        await Department.updateOne({ name: 'Department #21' }, { $set: { name: '=Department #1=' }});
        const updatedDepartment = await Department.findOne({ name: '=Department #1=' });
        expect(updatedDepartment).to.not.be.null;
    });
  
    it('should properly update one document with "save" method', async () => {
        const department = await Department.findOne({ name: 'Department #21' });
